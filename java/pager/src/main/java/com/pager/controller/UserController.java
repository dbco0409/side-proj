...
public class UserController {
    ...
    public Map<String, Object> LoginForm(@RequestParam("mbId") String mbId,
        ...
        if (user == null) {
            ...
            return result;
        } else if (!userService.checkPassword(mbPassword, user.getMbPassword())) {
            result.put("success", false);
            result.put("message", "아이디 혹은 비밀번호가 일치하지 않습니다.");
            return result;
        }
    ...
    @PostMapping("/forget1")
    @ResponseBody
	public Map<String, Object> forget1PostMethod(@RequestParam String mbId, @RequestParam String mbName, HttpSession session, Model model) {
        ...
        if (user == null) {
            result.put("success", false);
            result.put("message", "아이디 혹은 이름을 찾을 수 없습니다.");
            return result;
        }
    ...
    @GetMapping("/forget2")
    public String forget2RequestMethod(@RequestParam(required = false) String mbId, @RequestParam(required = false) String mbName, 
    ...
    @PostMapping("/forget2")
    @ResponseBody
	public Map<String, Object> forget2PostMethod(@RequestParam String mbId,@RequestParam String mbName, @RequestParam String mbPassword, @RequestParam("mbPasswordRe") String mbPasswordRe, HttpSession session, Model model) {
    	Map<String, Object> result = new HashMap<>();
    	try {
    	User user = userService.findByMbId(mbId, mbName);
        if (user == null) {
    ...
    public Map<String, Object> edit(
            ...
            @RequestParam String mbPassword,          // 현재 비밀번호
            @RequestParam String mbPasswordNew,   // 새 비밀번호
